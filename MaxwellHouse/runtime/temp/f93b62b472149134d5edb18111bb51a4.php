<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:81:"/www/web/zhu1_51zuopin_com/public_html/../application/admin/view/index/index.html";i:1528958402;s:81:"/www/web/zhu1_51zuopin_com/public_html/../application/admin/view/common/base.html";i:1528958400;}*/ ?>
...
<aside class="Hui-aside">
	...
	<?php if(is_array($result) || $result instanceof \think\Collection || $result instanceof \think\Paginator): if( count($result)==0 ) : echo "" ;else: foreach($result as $k=>$v): if(in_array(($v['menu_id']), is_array($menu_id)?$menu_id:explode(',',$menu_id))): ?>
		<dl id="menu-article">
			<dt><i class="Hui-iconfont">&#xe616;</i> <?php echo $v['menu_name']; ?><i class="Hui-iconfont menu_dropdown-arrow">&#xe6d5;</i></dt>
			<dd>
				<ul>
				<?php if(is_array($v['next']) || $v['next'] instanceof \think\Collection || $v['next'] instanceof \think\Paginator): if( count($v['next'])==0 ) : echo "" ;else: foreach($v['next'] as $k1=>$vo): if(in_array(($vo['menu_id']), is_array($menu_id)?$menu_id:explode(',',$menu_id))): ?>
					<li><a data-href="<?php echo url($vo['controller'].'/'.$vo['action']); ?>" data-title="<?php echo $vo['menu_name']; ?>" href="javascript:void(0)"><?php echo $vo['menu_name']; ?></a></li>
...
<!--请在下方写此页面业务相关的脚本-->
<script type="text/javascript" src="__STATIC__/__ADMIN__lib/jquery.contextmenu/jquery.contextmenu.r2.js"></script>
