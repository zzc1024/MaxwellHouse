<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:87:"/www/web/zhu1_51zuopin_com/public_html/../application/admin/view/exhibitions/index.html";i:1528958402;s:81:"/www/web/zhu1_51zuopin_com/public_html/../application/admin/view/common/base.html";i:1528958400;}*/ ?>
...
<div class="page-container">
	...
	<div class="cl pd-5 bg-1 bk-gray mt-20">
		<span class="l">
		<a class="btn btn-primary radius" onclick="system_banner_add('添加水印','<?php echo url('exhibitions/add'); ?>')" href="javascript:;"><i class="Hui-iconfont">&#xe600;</i> 添加水印</a>
		</span>
	</div>
	<div class="mt-20">
		<table class="table table-border table-bordered table-hover table-bg table-sort">
			<thead>
				<tr class="text-c">
					<th width="120">展示栏目</th>
					<th width="100">展示名称</th>
					<th width="100">商品编号</th>
					<th width="100">展示价格</th>
					<th >水印图片</th>
			...
			<tbody>
				...
				<tr class="text-c">
					...
					<?php endif; ?>
					<a title="编辑"  onclick="system_banner_edit('水印编辑','<?php echo url('exhibitions/edit','id='.$v['id']); ?>')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i></a> <a title="删除" href="javascript:;" onclick="system_banner_del(<?php echo $v['id']; ?>)" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6e2;</i></a></td>
				</tr>
...
/*系统-栏目-添加*/
function system_banner_add(title,url){
...
/*系统-栏目-编辑*/
function system_banner_edit(title,url){
	var index = layer.open({
		type: 2,
		title: title,
		content: url
...
function system_banner_del(w){
	...
	layer.confirm('确认要删除吗？',function(){
		$.ajax({
			type: 'POST',
			url: '<?php echo url('exhibitions/exhibition_delete'); ?>',
			dataType: 'json',
			data:data,
...
/*停用*/
function admin_stop(obj,id,y,x){
	var data = {
		'id':y,
		'statu':'0',
	}
	layer.confirm('确认要停用吗？',function(){
		$.ajax({
			type: 'POST',
			url: '<?php echo url('Exhibitions/exhibition_statu'); ?>',
			dataType: 'json',
			data: data,
